...
/* This is broken ahhhh implement later	
private void movePlayers() {
		for (int i = 0; i < inputs.length; i++) {
			...
			if (validMovement(players[i], input)) {
				// Move the player to the new position
				Position newPos = Position.calcPosition(
						players[i].getPosition(), input);
